...
describe('JustificationService', () => {
  ...
  it('should be left justified when having only one line', () => {
    const input = new WordSplitter().split('text    one left', JUSTIFICATION_LINE_LENGTH);
  ...
  it('should be justified when having only one word', () => {
    ...
    const input = new WordSplitter().split('text   '.repeat(10), JUSTIFICATION_LINE_LENGTH);
  ...
  it('should be split into multiple lines when word exceeds line length', () => {
    ...
    const input = new WordSplitter().split('text'.repeat(30), JUSTIFICATION_LINE_LENGTH);
  ...
  it('should have two lines, the first is full justified and the second (last) is left justified', () => {
    const input = new WordSplitter().split('abc '.repeat(30), JUSTIFICATION_LINE_LENGTH); // 120 characters => max is 80 per line
